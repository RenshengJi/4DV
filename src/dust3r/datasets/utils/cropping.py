...
def rescale_image_depthmap_flowmap(
    image, depthmap, flowmap, camera_intrinsics, output_resolution, force=True
):
    """Jointly rescale a (image, depthmap, flowmap)
    so that (out_width, out_height) >= output_res
    """
    image = ImageList(image)
    input_resolution = np.array(image.size)  # (W,H)
...
def rescale_image_depthmap_flowmap_segmask(
    ...
    """Jointly rescale a (image, depthmap, flowmap, seg_mask)
    so that (out_width, out_height) >= output_res
    """
    image = ImageList(image)
    input_resolution = np.array(image.size)  # (W,H)
...
def crop_image_depthmap_flowmap(image, depthmap, flowmap, camera_intrinsics, crop_bbox):
    """
    Return a crop of the input view including flowmap.
    """
    image = ImageList(image)
    l, t, r, b = crop_bbox
...
def crop_image_depthmap_flowmap_segmask(image, depthmap, flowmap, seg_mask, camera_intrinsics, crop_bbox):
    """
    Return a crop of the input view including flowmap and seg_mask.
    """
    image = ImageList(image)
    l, t, r, b = crop_bbox
